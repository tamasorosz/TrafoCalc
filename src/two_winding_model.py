...
class TwoWindingModel:
    ...
    def calculate(self, is_sc=False):
        ...
        if not is_sc:
            ...
            self.results.load_loss += cryo_loss + thermal_loss
            # the product price is approximated by the
            product_price = self.input.costs.hv_cost * self.hv_winding.cable_length + self.input.costs.lv_cost * self.lv_winding.cable_length + self.input.costs.core_cost * self.results.core_mass
