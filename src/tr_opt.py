...
from optimization_functions import (
    Dependent_Params,
    Parameters,
    calc_dependent_variables,
    eval_objective,
    fem_analytics_improved_model,
...
def run_fem(param, dep, indep):
    ...
    r_in = dep.r_in - dep.t_in / 2.0
    r_ou = dep.r_ou - dep.t_ou / 2.0
    r_reg = dep.r_reg - dep.t_reg / 2.0

    create_winding(magnetic, geometry, r_in, param.ei / 2.0, dep.t_in, indep.h_in, "lv", param.ff_in, indep.j_in * 2.0**0.5)
    create_winding(magnetic, geometry, r_ou, param.ei / 2.0, dep.t_ou, dep.h_ou, "hv", param.ff_ou, -indep.j_ou * 2.0**0.5)

    # checks that the regulating winding is activated or not
    if param.reg_range > 1e-6:
        create_winding(magnetic, geometry, r_reg, param.ei / 2.0, dep.t_reg, dep.h_reg, "reg", param.ff_reg, 0.0)
    ...
    sb = param.power / 1000.0  # nominal power  --- MVA
    ...
    Omega = 2.0 * pi * param.freq

    # -- calculating the impedance from the volume integrals --
    L = 2 * solution.volume_integrals()["Wm"] / ib**2.0
    dep.sci = 2.0 * pi * param.freq * L / zb * 100.0
    ...
    Bg = b_gap(
        n_in, ib, indep.h_in
    )  # TODO <--- should be changed by the maximum of the inductance in the gap --- at least ...

    # inner winding -- ctc
    status, m, objval = gp_model_winding(
        n_in,
        dep.r_in + dep.t_in / 2.0,
        dep.t_in,
        indep.h_in,
        param.in_ins_rad,
        param.in_ins_s,
    ...
    for i in range(0, nx):
        xx = dep.r_ou * 1e-3 + i * dx
        for j in range(0, ny):
            yy = param.ei / 2.0 * 1e-3 + j * dy * 0.2 + ny * dy * 0.8  # upper segment
...
class TestProblem(Problem):
    def __init__(self, name):
        parameters = {
            "r_c": {"initial_value": 250.0, "bounds": [200.0, 600.0]},
            "b_c": {"initial_value": 1.6, "bounds": [1.4, 1.7]},
            "j_in": {"initial_value": 2.0, "bounds": [1.5, 3.5]},
            "j_ou": {"initial_value": 2.0, "bounds": [1.5, 3.5]},
            "j_reg": {"initial_value": 3.0, "bounds": [2.0, 3.8]},
            "h_in": {"initial_value": 1500.0, "bounds": [800.0, 3000.0]},
            "m_gap": {"initial_value": 37.0, "bounds": [37.0, 100.0]},
        }
        costs = ["F"]
    ...
    def evaluate(self, x: list):
        ...
        ind = independent_variables(r_c=x[0], b_c=x[1], j_in=x[2], j_ou=x[3], j_reg=x[4], h_in=x[5], m_gap=x[6])
        ...
        p.gap_core = 20.0
        p.ei = 150.0
        p.phase_distance = 37.0
        p.ff_c = 0.9
        p.cc = 3.5
        p.u_in_line = 33.0  # line voltage in the inner winding
        p.u_in = 19.05
        p.u_out = 132.0
        p.ff_in = 0.6
        p.ff_ou = 0.60
        ...
        p.con_fact_in = 1.0
        p.con_fact_ou = 1.73
        p.u_out = 132.0  # phase voltage on the outer terminal [kV]
        p.u_out_line = 132.0
...
class TestProblem2(Problem):
    def __init__(self, name):
        parameters = [
            {"name": "r_c", "bounds": [150.0, 300.0]},
            {"name": "b_c", "initial_value": 1.6, "bounds": [1.4, 1.7]},
            {"name": "j_in", "initial_value": 2.0, "bounds": [1.5, 3.0]},
            {"name": "j_ou", "initial_value": 2.0, "bounds": [1.5, 3.0]},
            {"name": "j_reg", "initial_value": 3.0, "bounds": [2.0, 3.5]},
            {"name": "h_in", "initial_value": 1500.0, "bounds": [1000.0, 2000.0]},
            {"name": "m_gap", "initial_value": 37.0, "bounds": [24.0, 70.0]},
        ]
    ...
    def evaluate(self, x: list):
        ...
        p.con_fact_in = 1.0
        p.con_fact_ou = 1.0
        p.u_out = 33.0 / 1.73  # phase voltage on the outer terminal [kV]
        p.u_out_line = 33.0
...
class TestProblem3(Problem):
    def set(self):
        ...
        self.working_dir = "./"
        self.parameters = [
            {"name": "r_c", "bounds": [150.0, 400.0]},
            {"name": "b_c", "initial_value": 1.6, "bounds": [1.4, 1.7]},
            {"name": "j_in", "initial_value": 2.0, "bounds": [1.5, 3.5]},
            {"name": "j_ou", "initial_value": 2.0, "bounds": [1.5, 3.5]},
            {"name": "j_reg", "initial_value": 3.0, "bounds": [2.0, 3.8]},
            {"name": "h_in", "initial_value": 1500.0, "bounds": [1000.0, 2500.0]},
            {"name": "m_gap", "initial_value": 37.0, "bounds": [38.0, 70.0]},
        ]
    ...
    def evaluate(self, individual):
        ...
        p.ff_c = 0.89
        p.cc = 3.5
        p.u_in_line = 20.0  # line voltage in the inner winding
        p.u_in = 20.0
        p.ff_in = 0.65
        p.ff_ou = 0.60
        p.win_c = 10.0
        ...
        p.con_fact_in = 1.0
        p.con_fact_ou = 1.0
        p.u_out = 130.0 / 1.73  # phase voltage on the outer terminal [kV]
        p.u_out_line = 130.0
...
def Test_6300kVA_transformer():
    ...
    indep = ind = independent_variables(r_c=184.0, b_c=1.568, j_in=3.02, j_ou=3.0, j_reg=1.73, h_in=979.0, m_gap=26.7)
    #
    ...
    p.ff_c = 0.838
    p.cc = 3.5
    p.u_in_line = 22.0  # line voltage in the inner winding
    p.u_in = 22.0
    p.ff_in = 0.535
    p.ff_ou = 0.56
    p.win_c = 10.0
