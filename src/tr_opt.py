...
def create_rectangle(geo, x0, y0, width, height, boundary=None):
    """
    ...
    if boundary is not None:
        ...
        geo.add_edge(x0, y0, x0 + width, y0, boundaries=boundary)
        geo.add_edge(x0 + width, y0, x0 + width, y0 + height, boundaries=boundary)
        geo.add_edge(x0 + width, y0 + height, x0, y0 + height, boundaries=boundary)
        geo.add_edge(x0, y0 + height, x0, y0, boundaries=boundary)

    else:

        geo.add_edge(x0, y0, x0 + width, y0)
        geo.add_edge(x0 + width, y0, x0 + width, y0 + height)
        geo.add_edge(x0 + width, y0 + height, x0, y0 + height)
        geo.add_edge(x0, y0 + height, x0, y0)

    return x0 + width / 2., y0 + height / 2.  # gives back the center of the rectangle
...
def run_fem_simple(param, dep, indep):
    """
    :param param: problem parameters
    :param dep: dependent variables
    :param indep: independent variables
